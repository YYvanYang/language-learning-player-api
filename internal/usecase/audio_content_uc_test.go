// ==================================================
// FILE: internal/usecase/audio_content_uc_test.go (MODIFIED)
// ==================================================
...
import (
	...
	"fmt"
	"io"
	"log/slog" // Added for URL parsing
...
// Helper to create a dummy full config (including CDN)
func newTestFullConfig(cdnBaseURL string) config.Config {
	return config.Config{
		Minio: config.MinioConfig{BucketName: "test-bucket", PresignExpiry: 1 * time.Hour},
		CDN:   config.CDNConfig{BaseURL: cdnBaseURL},
		// Add other necessary default configs if usecase constructor needs them
		JWT: config.JWTConfig{AccessTokenExpiry: 1 * time.Hour},
	}
}

func TestAudioContentUseCase_GetAudioTrackDetails_Success_NoCDN(t *testing.T) {
	...
	cfg := newTestFullConfig("") // No CDN Base URL

	uc := usecase.NewAudioContentUseCase(cfg, mockTrackRepo, mockCollectionRepo, mockStorageService, mockTxManager, logger)

	trackID := domain.NewTrackID()
	expectedTrack := &domain.AudioTrack{ID: trackID, Title: "Test", MinioBucket: cfg.Minio.BucketName, MinioObjectKey: "test/key"}
	expectedURL := "http://minio.example.com/presigned-url?sig=123" // Original presigned URL

	// Mock expectations
	mockTrackRepo.On("FindByID", mock.Anything, trackID).Return(expectedTrack, nil).Once()
	mockStorageService.On("GetPresignedGetURL", mock.Anything, cfg.Minio.BucketName, expectedTrack.MinioObjectKey, cfg.Minio.PresignExpiry).Return(expectedURL, nil).Once()
	...
	assert.Equal(t, expectedURL, playURL) // Should return the original URL
	mockTrackRepo.AssertExpectations(t)
	mockStorageService.AssertExpectations(t)
}

func TestAudioContentUseCase_GetAudioTrackDetails_Success_WithCDN(t *testing.T) {
	mockTrackRepo := mocks.NewMockAudioTrackRepository(t)
	mockCollectionRepo := mocks.NewMockAudioCollectionRepository(t)
	mockStorageService := mocks.NewMockFileStorageService(t)
	mockTxManager := mocks.NewMockTransactionManager(t)
	logger := newTestLoggerForAudioUC()
	cdnBaseURL := "https://cdn.my-audio.com"
	cfg := newTestFullConfig(cdnBaseURL) // Configure CDN Base URL

	uc := usecase.NewAudioContentUseCase(cfg, mockTrackRepo, mockCollectionRepo, mockStorageService, mockTxManager, logger)

	trackID := domain.NewTrackID()
	minioBucket := "internal-bucket"
	minioKey := "audio/track1.mp3"
	expectedTrack := &domain.AudioTrack{ID: trackID, Title: "Test CDN", MinioBucket: minioBucket, MinioObjectKey: minioKey}
	originalPresignedURL := "http://local-minio:9000/" + minioBucket + "/" + minioKey + "?X-Amz-Algorithm=...&X-Amz-Signature=..." // Example original URL
	expectedRewrittenURL := cdnBaseURL + "/" + minioBucket + "/" + minioKey + "?" + strings.Split(originalPresignedURL, "?")[1]    // Expected rewritten URL (path + query params)

	// Mock expectations
	mockTrackRepo.On("FindByID", mock.Anything, trackID).Return(expectedTrack, nil).Once()
	mockStorageService.On("GetPresignedGetURL", mock.Anything, minioBucket, minioKey, cfg.Minio.PresignExpiry).Return(originalPresignedURL, nil).Once()
	...
	require.NoError(t, err)
	assert.Equal(t, expectedTrack, track)
	assert.Equal(t, expectedRewrittenURL, playURL) // Assert the URL was rewritten
	mockTrackRepo.AssertExpectations(t)
	mockStorageService.AssertExpectations(t)
}
...
func TestAudioContentUseCase_GetAudioTrackDetails_PresignError(t *testing.T) {
	...
	trackID := domain.NewTrackID()
	expectedTrack := &domain.AudioTrack{ID: trackID, MinioBucket: cfg.Minio.BucketName, MinioObjectKey: "test/key"}
	presignError := errors.New("failed to connect to storage")

	// Mock expectations
	mockTrackRepo.On("FindByID", mock.Anything, trackID).Return(expectedTrack, nil).Once()
	mockStorageService.On("GetPresignedGetURL", mock.Anything, cfg.Minio.BucketName, expectedTrack.MinioObjectKey, cfg.Minio.PresignExpiry).Return("", presignError).Once()
	...
	// Assert
	require.Error(t, err)
	assert.Contains(t, err.Error(), "could not retrieve base playback URL") // Check for wrapped error
	assert.Equal(t, expectedTrack, track)                                   // Track info should still be returned
	assert.Empty(t, playURL)                                                // Play URL is empty due to error
	mockTrackRepo.AssertExpectations(t)
	mockStorageService.AssertExpectations(t)
}
...
func TestAudioContentUseCase_CreateCollection_Success(t *testing.T) {
	...
	// Simulate tracks existing by returning a slice of the same length as input IDs
	mockTrackRepo.On("ListByIDs", mock.Anything, initialTrackIDs).Return(make([]*domain.AudioTrack, len(initialTrackIDs)), nil).Once()
...
func TestAudioContentUseCase_CreateCollection_TxFailureOnCreate(t *testing.T) {
	...
	mockTxManager.On("Execute", mock.Anything, mock.AnythingOfType("func(context.Context) error")).
		Run(func(args mock.Arguments) {
			...
			assert.ErrorIs(t, err, createError) // Check underlying error
			assert.Contains(t, err.Error(), "saving collection metadata")
		}).
		// TxManager Execute returns the wrapped error from the function
		Return(fmt.Errorf("failed to create collection: %w", fmt.Errorf("saving collection metadata: %w", createError))).
		Once()
	...
	assert.ErrorIs(t, err, createError)                            // Check original error is wrapped
...
// --- Remaining tests from the original file ---
// (TestCreateCollection_TxFailureOnManageTracks, TestCreateCollection_TrackValidationFails, etc.)
// These tests should already be compatible or require minimal changes.
// Ensure they correctly mock the transaction flow using mockTxManager.On("Execute", ...).
